...
async def _seed_players(client: Any, guild_id: int, count: int = 15) -> list[int]:
...
async def test_join_and_count(client: Any) -> None:
...
async def test_double_join_rejected(client: Any) -> None:
    await _setup_guild(client)
...
async def test_join_rejects_player_from_another_guild(client: Any) -> None:
...
async def test_leave(client: Any) -> None:
    await _setup_guild(client)
...
async def test_is_full(client: Any) -> None:
    await _setup_guild(client)
...
async def test_status_returns_count_and_average_elo(client: Any) -> None:
    await _setup_guild(client)
    repo = PlayerRepository(client)
...
async def test_pop_all_clears(client: Any) -> None:
    await _setup_guild(client)
    ids = await _seed_players(client, 100, count=15)
    svc = QueueService(client, queue_size=15)
    for pid in ids:
...
async def test_claim_match_requires_full_queue_without_removing_players(client: Any) -> None:
    await _setup_guild(client)
...
async def test_claim_match_removes_only_configured_queue_size(client: Any) -> None:
    await _setup_guild(client)
...
async def test_claim_match_uses_guild_queue_size(client: Any) -> None:
    repo = GuildRepository(client)
